...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('ℹ️ ʀᴜʟᴇꜱ 🌠', url=GRP_LNK),
                    InlineKeyboardButton('🎥🍿 ᴍᴏᴠɪᴇ🍿🎥', url=CHNL_LNK)
                ]]
            ...
            try:
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          InlineKeyboardButton('🍿 ᴜᴘᴅᴀᴛᴇꜱ 🎥', url="https://t.me/trelairpark13")
                         ]]
                    )
                )
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    ...
                    InlineKeyboardButton("⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url='https://t.me/+jg74vZvDiY9iNjA1')
                ]]
                await message.reply_text(
            ...
            msg = await client.send_cached_media(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                     [
                      InlineKeyboardButton('🍿 ᴜᴘᴅᴀᴛᴇꜱ 🎥', url="hhttps://t.me/trelairpark13")
                     ]]
                )
            )
            filesarr.append(msg)
    ...
    if not files_:
        ...
        try:
            ...
            title = 'movie ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@Thunnder_Bot ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if settings is not None:
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton(
                    '10 Mɪɴs' if settings["auto_delete"] else '❌',
                    callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{grp_id}',
                ),
            ],
            [
...
async def shortlink(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(f"<b>Hey {message.from_user.mention}, This command only works on groups !\n\n<u>Follow These Steps to Connect Shortener:</u>\n\n1. Add Me in Your Group with Full Admin Rights\n\n2. After Adding in Grp, Set your Shortener\n\nSend this command in your group\n\n—> /shortlink ""{your_shortener_website_name} {your_shortener_api}\n\n#Sample:-\n/shortlink droplink.com 1f1da5c9df9a58058672ac8d8134e2....\n\nThat's it!!! Enjoy Earning Money 💲\n\nIf you have any Doubts, Feel Free to Ask me - @MS_Contact_Robot</b>")
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
